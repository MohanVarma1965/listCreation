...
debugger;
if(myStorage.getItem('resp') == null){
	myStorage.setItem('resp',JSON.stringify(resp));
}


let st = localStorage.getItem('resp');
console.log("st")
console.log(st);

let localObject = {};
Object.assign(localObject,JSON.parse(st));
...
function renderModalAdd() {
	...
	<div class="modal-dialog" role="document"><div class="modal-content"><div class="modal-header">
		<h5 class="modal_Add">Add a New Graduate Attribute</h5>  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
			...
			<input type="text" class="form-control" aria-describedby="inputGroup-sizing-default" value="" onchange="change(e)"> </input>	
			</div>
...
function renderModalEdit(data) {
	...
	<div class="modal-dialog" role="document"><div class="modal-content"><div class="modal-header">
		<h5 class="modal_Add">Modify Graduate Attribute</h5>  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
			...
			<input id="modalID" type="text" class="form-control" aria-label="Default" aria-describedby="inputGroup-sizing-default" value="` + data.title + `">	
			</div>
			<div class="modal-footer override_ftr" > 
			<div><button type="button" class="btn btn-outline-danger">Delete</button></div>
			<div><button type="button" class="btn btn-primary" onclick="modify()">Save</button>
...
function fakeGetRequest() {
	...
	//let rawdata = fs.readFileSync('JSON repsonse.json');
...
function modify() {
	...
	let  modalDefault = modalNode.defaultValue;
	let modifiedVlaue = modalNode.value;


	let arr = localObject['GA'];

debugger;

	for (let i in arr) {
		if (arr[i].title == modalDefault) {
			arr[i].title = modifiedVlaue;
			break; //Stop this loop, we found it!
		} else {
			for( let j in arr[i].sub_ga) {
				if(arr[i].sub_ga[j].title == modalDefault) {
					arr[i].sub_ga[j].title = modifiedVlaue;
					break;
				}
			}
		}
	...
	localStorage.setItem('resp',JSON.stringify(localObject));
